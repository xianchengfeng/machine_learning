...
def loadDataSet():
	...
	for line in fr.readlines():
		lineArr = line.strip().split() #删除空白字符
...
def plotBestFit(wei):
	...
	fig = plt.figure()
	ax = fig.add_subplot(111) #多少行，多少列，第几副图
...
def multiTest():
	...
	for k in range(numTests):
		errorSum += colicTest()
	print("after %d iterations th average error rate is : %f" % (numTests,errorSum/float(numTests)))
